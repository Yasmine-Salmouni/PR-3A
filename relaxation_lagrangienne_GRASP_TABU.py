...
# =============================================================================
# Heuristiques combinées GRASP + TABU (corrigées)
# =============================================================================
...
def tabu_search_multiops_fast(
    ...
    """Recherche tabou multi-opérateurs corrigée."""
    ...
    for _ in range(int(iterations)):
        if start_from.lower().strip() == "greedy":
            start = greedy_construct(
                ...
                alpha_urgency=alpha_urgency, lambda_campaign=lambda_campaign
            )
        else:
        ...
        improved = tabu_search_multiops_fast(
            start, stock_proj_chute, reincorpo_maxi, options, slots,
            lambda_campaign, rng,
        ...
        _, _, obj = evaluate_solution(improved, stock_proj_chute, reincorpo_maxi, lambda_campaign)

        if obj > best_obj + 1e-12:
            best_obj = obj
            best_assignment = dict(improved)

    return best_assignment, best_obj


# =============================================================================
# Relaxation Lagrangienne avec GRASP + TABU
# =============================================================================

def solve_lagrangian_grasp_tabu(stock_file, plan_file, seuil_reincorpo_mini=100.0,
                              alpha_init=0.1, epsilon=0.01, max_iterations=100,
                              # Paramètres GRASP
                              grasp_iterations=6, rcl=3, alpha_urgency=10.0,
                              lambda_campaign=1.0, seed=0, rcl_bias=0.70,
                              # Paramètres TABU
                              tabu_iters=50, tabu_tenure=10, start_from="greedy",
                              inactive_pool=15, neighbor_samples=18,
                              candidate_chutes_topk=3, swap_pairs=8, reloc_pairs=8,
                              cache_size=6000, patience=15, enable_replace=True,
                              enable_swap=True, enable_relocation=False,
                              reactive_tabu=True, tabu_tenure_min=5,
                              tabu_tenure_max=20, revisit_window=10,
                              freq_penalty_weight=0.02, restart_after=14,
                              intensify_after=5, intensify_neighbors=10,
                              intensify_swap_pairs=6):
    """
    Résout le modèle de ré-incorporation par relaxation lagrangienne
    avec heuristique GRASP + TABU pour la réparation.
    """
    ...
    print("=" * 70)
    print("RELAXATION LAGRANGIENNE — HEURISTIQUE GRASP + TABU")
    print("=" * 70)
    print(f"  Nombre de slots (p, dp)           : {len(slots)}")
    print(f"  Seuil mini (kg)                   : {seuil_reincorpo_mini}")
    print(f"  GRASP + TABU iterations          : {grasp_iterations}")
    print(f"  GRASP RCL size                   : {rcl}")
    ...
    # -----------------------------------------------------------------
    # Step 1 & 2 : Solution initiale valide avec GRASP + TABU
    # -----------------------------------------------------------------
    grasp_tabu_assignment, grasp_tabu_obj = grasp_tabu(
        stock_proj_chute, reincorpo_maxi, options, slots, seuil_reincorpo_mini,
        ...
        tabu_iters=tabu_iters, tabu_tenure=tabu_tenure, start_from=start_from,
        inactive_pool=inactive_pool, neighbor_samples=neighbor_samples,
        candidate_chutes_topk=candidate_chutes_topk, swap_pairs=swap_pairs,
        reloc_pairs=reloc_pairs, cache_size=cache_size, patience=patience,
        enable_replace=enable_replace, enable_swap=enable_swap,
    ...
    Z_best_primal = grasp_tabu_obj
    best_assignment = dict(grasp_tabu_assignment)

    print(f"  Solution initiale GRASP+TABU Z_best_primal : {Z_best_primal:.2f}")
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # =============================================================
        # Step 7 : Heuristique GRASP + TABU pour réparation (appliquée à chaque itération)
        # =============================================================
        grasp_tabu_repair, grasp_tabu_repair_obj = grasp_tabu(
            stock_proj_chute, reincorpo_maxi, options, slots, seuil_reincorpo_mini,
            iterations=grasp_iterations, rcl=rcl, alpha_urgency=alpha_urgency,
        ...
        if grasp_tabu_repair_obj > Z_best_primal:
            Z_best_primal = grasp_tabu_repair_obj
            best_assignment = dict(grasp_tabu_repair)
    ...
    print(f"  Heuristique utilisée        : GRASP+TABU (GRASP_iters={grasp_iterations}, TABU_iters={tabu_iters})")
    print(f"  Start from                   : {start_from}")
    print()
...
if __name__ == "__main__":
    ...
    solve_lagrangian_grasp_tabu(stock_file, plan_file, seuil_mini)
