...
# =============================================================================
# Heuristiques combinées GRASP + LNS
# =============================================================================
...
# =============================================================================
# Relaxation Lagrangienne avec GRASP + LNS
# =============================================================================

def solve_lagrangian_grasp_lns(stock_file, plan_file, seuil_reincorpo_mini=100.0,
                              alpha_init=0.1, epsilon=0.01, max_iterations=100,
                              # Paramètres GRASP
                              grasp_iterations=10, rcl=3, alpha_urgency=10.0,
    ...
    avec heuristique GRASP + LNS pour la réparation.
    """
    ...
    print("=" * 70)
    print("RELAXATION LAGRANGIENNE — HEURISTIQUE GRASP + LNS")
    print("=" * 70)
    print(f"  Nombre de slots (p, dp)           : {len(slots)}")
    print(f"  Seuil mini (kg)                   : {seuil_reincorpo_mini}")
    print(f"  GRASP + LNS iterations           : {grasp_iterations}")
    ...
    # -----------------------------------------------------------------
    # Step 1 & 2 : Solution initiale valide avec GRASP + LNS
    # -----------------------------------------------------------------
    grasp_lns_assignment, grasp_lns_obj = grasp_lns(
        ...
        start_from=start_from, lns_iters=lns_iters, lns_k=lns_k, lns_rcl=lns_rcl,
        lns_rcl_bias=lns_rcl_bias, lns_alpha_urgency=lns_alpha_urgency,
        patience=patience, accept_non_improving=accept_non_improving
    )
    
    Z_best_primal = grasp_lns_obj
    best_assignment = dict(grasp_lns_assignment)

    print(f"  Solution initiale GRASP+LNS Z_best_primal : {Z_best_primal:.2f}")
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # =============================================================
        # Step 7 : Heuristique GRASP + LNS pour réparation (appliquée à chaque itération)
        # =============================================================
        grasp_lns_repair, grasp_lns_repair_obj = grasp_lns(
            stock_proj_chute, reincorpo_maxi, options, slots, seuil_reincorpo_mini,
            iterations=grasp_iterations, rcl=rcl, alpha_urgency=alpha_urgency,
        ...
        if grasp_lns_repair_obj > Z_best_primal:
            Z_best_primal = grasp_lns_repair_obj
            best_assignment = dict(grasp_lns_repair)
    ...
    print(f"  Heuristique utilisée        : GRASP+LNS (GRASP_iters={grasp_iterations}, LNS_iters={lns_iters})")
    print(f"  Start from                   : {start_from}")
    print()
...
if __name__ == "__main__":
    ...
    solve_lagrangian_grasp_lns(stock_file, plan_file, seuil_mini)
