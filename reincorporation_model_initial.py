...
def solve_reincorporation(stock_file, plan_file, seuil_reincorpo_mini=100.0):
    ...
    print("=" * 70)
    print("MODÈLE DE RÉ-INCORPORATION DES CHUTES")
    print("=" * 70)
    ...
    if solver is None:
        raise RuntimeError("Aucun solveur MIP disponible (SCIP ou CBC).")
    ...
    for (p, dp, c), maxi in reincorpo_maxi.items():
        ...
        for dc in chute_windows[c]:
            ...
            if dmc <= dp <= dlc: #Contrainte: Respect de la fenêtre d’utilisation de la chute
                ...
                var = solver.NumVar(0.0, upper, name)
                key = (c, dc, p, dp)
                R[key] = var
    ...
    for (p, dp, c) in reincorpo_maxi:
        ...
        if (p, dp, c) in R_by_p_dp_c:
            O[(p, dp, c)] = solver.BoolVar(f"O_{p}_{dp.strftime('%m%d')}_{c}")
    ...
    for (c, dc), stock_val in stock_proj_chute.items():
        ...
        if keys:
            solver.Add(
                solver.Sum([R[k] for k in keys]) <= stock_val,
                f"stock_{c}_{dc[0].strftime('%m%d')}_{dc[1].strftime('%m%d')}",
            )
            cnt += 1
    nb["stock"] = cnt
    ...
    for (p, dp, c), maxi in reincorpo_maxi.items():
        ...
        solver.Add(
            solver.Sum([R[k] for k in keys]) <= maxi * O[(p, dp, c)],
            f"cap_{p}_{dp.strftime('%m%d')}_{c}",
        )
        cnt += 1
    nb["capacité"] = cnt
    ...
    cnt = 0
    for (p, dp, c) in list(O):
        keys = R_by_p_dp_c[(p, dp, c)]
        solver.Add(
            seuil_reincorpo_mini * O[(p, dp, c)] <= solver.Sum([R[k] for k in keys]),
            f"seuil_{p}_{dp.strftime('%m%d')}_{c}",
        )
        cnt += 1
    nb["seuil"] = cnt
    ...
    for (p, dp), chutes in pd_chutes.items():
        solver.Add(
            solver.Sum([O[(p, dp, c)] for c in chutes]) <= 1,
            f"unique_{p}_{dp.strftime('%m%d')}",
        )
        cnt += 1
    nb["unicité"] = cnt
    ...
    #
    # R est indexé par (c,dc,p,dp).  O est indexé par (p,dp,c) mais
    # apparaît dans la somme sur dc : pour chaque (c,dc,p,dp) valide,
    # O[p,dp,c] est soustrait une fois → coefficient = −(nb de dc valides).
    ...
    for key_o, coeff in o_coeff.items():
        if key_o in O:
            objective.SetCoefficient(O[key_o], coeff)
    ...
    # 7. Résultats
    # -----------------------------------------------------------------
    total_reincorpore = sum(v.solution_value() for v in R.values())
    ...
    nb_activations = sum(1 for v in O.values() if v.solution_value() > 0.5)
    #Le nombre d'activations

    print("=" * 70)
    print("RÉSULTATS")
    print("=" * 70)
    print(f"  Valeur objectif          : {solver.Objective().Value():.2f}")
